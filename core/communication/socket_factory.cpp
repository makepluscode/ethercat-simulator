...
void SocketFactory::registerDefaultCreators()
{
    ...
    registerCreator("tcp", [](const std::string& endpoint) -> int {
        ...
        if (::connect(sock, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) < 0) {
            ::close(sock);
            throw std::runtime_error("Failed to connect to TCP endpoint: " + std::string(strerror(errno)));
        }
    ...
    registerCreator("uds", [](const std::string& endpoint) -> int {
        ...
        if (::connect(sock, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) < 0) {
            ::close(sock);
            throw std::runtime_error("Failed to connect to UDS endpoint: " + std::string(strerror(errno)));
        }
