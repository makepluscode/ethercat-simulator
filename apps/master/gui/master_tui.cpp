                    ...
                    std::shared_ptr<MasterModel> model, bool smoke_test)
{
    ...
    auto do_read   = Button("Read",
                            [&]
                            {
                              ...
                              controller->sdoUpload(selected, static_cast<uint16_t>(idx),
                                                      static_cast<uint8_t>(sub), value);
                          });
    auto do_write =
        Button("Write",
               [&]
               {
                   ...
                   controller->sdoDownload(selected, static_cast<uint16_t>(idx),
                                           static_cast<uint8_t>(sub), static_cast<uint32_t>(v));
               });
    ...
    auto renderer =
        Renderer(inputs,
                 [&]
                 {
                     ...
                     rows.push_back(hbox({text("Addr") | bold | underlined, text("  "),
                                          text("State") | bold | underlined, text("  "),
                                          text("AL") | bold | underlined}));
                     for (size_t i = 0; i < snap.slaves.size(); ++i)
                     {
                         auto const& r = snap.slaves[i];
                         auto line     = hbox({text(std::to_string(r.address)), text("  "),
                                               text(r.state), text("  "), text(r.al_code)});
    ...
    auto events = CatchEvent(renderer,
                             [&](Event e)
                             {
