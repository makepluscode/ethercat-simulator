...
namespace kickcat {
enum class ProcessingResult { NOOP, CONTINUE, FINALIZE, FINALIZE_AND_KEEP };
...
class AbstractMessage {
  public:
    /// \param mailbox_size Size of the mailbox the message is targeted to (required to adapt
    /// internal buffer)
    ...
    /// \return CONTINUE if the message is related and operation requiered another loop (message
    /// shall be push again in sending queue)
    virtual ProcessingResult process(uint8_t const* received) = 0;

    /// Handle address field. Address meaning depends on context (0 for local processing, slave
    /// address for gateway processing)
    void setAddress(uint16_t address) {
        header_->address = address;
    }
    ...
    /// \param current_time     Considered time to compute message status (enable injection for
    /// tests) \return current message status. Value may depend on underlying service.
  ...
  private:
    nanoseconds timeout_; // Max time to handle the message. Relative time before sending, absolute
                          // time after. 0 means no timeout
};
...
class GatewayMessage : public AbstractMessage {
  public:
    ...
    uint16_t         gatewayIndex() const {
        return gateway_index_;
    }

  private:
    uint16_t address_; // Requested address: store it to set it back after processing because the
                       // header field is used to store gateway index
...
struct Mailbox {
    ...
    // messages factory
    std::shared_ptr<AbstractMessage> createSDO(uint16_t index, uint8_t subindex, bool CA,
                                               uint8_t request, void* data, uint32_t* data_size,
                                               nanoseconds timeout = 20ms);
    std::shared_ptr<GatewayMessage>  createGatewayMessage(uint8_t const* raw_message,
                                                          int32_t        raw_message_size,
    ...
    /// \param current_time     Considered time to process message timeout (enable injection for
    /// tests)
    ...
    std::list<std::shared_ptr<AbstractMessage>>
        to_process; // message already sent, waiting for an answer
...
class SDOMessage : public AbstractMessage {
  public:
    SDOMessage(uint16_t mailbox_size, uint16_t index, uint8_t subindex, bool CA, uint8_t request,
  ...
  protected:
    ProcessingResult processUpload(mailbox::Header const* header, CoE::ServiceData const* sdo,
                                   uint8_t const* payload);
    ProcessingResult processUploadSegmented(mailbox::Header const*  header,
                                            CoE::ServiceData const* sdo, uint8_t const* payload);
    ProcessingResult processDownload(mailbox::Header const* header, CoE::ServiceData const* sdo,
                                     uint8_t const* payload);
    ProcessingResult processDownloadSegmented(mailbox::Header const*  header,
                                              CoE::ServiceData const* sdo, uint8_t const* payload);
