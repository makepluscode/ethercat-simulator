...
void sendGetRegister(Link& link, uint16_t slave_address, uint16_t reg_address, T& value_read) {
    ...
    link.addDatagram(Command::FPRD, createAddress(slave_address, reg_address), nullptr, process,
                     error);
...
void sendWriteRegister(Link& link, uint16_t slave_address, uint16_t reg_address, T value_write) {
    ...
    link.addDatagram(Command::FPWR, createAddress(slave_address, reg_address), value_write, process,
                     error);
    link.processDatagrams();
}
} // namespace kickcat
