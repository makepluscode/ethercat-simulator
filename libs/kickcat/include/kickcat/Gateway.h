...
class Gateway
{
  public:
    ...
    /// \details Try to get a request from the network (non blocking call). If there is data, check
    /// the request coherency then
    ///          transfer it to the EtherCAT bus (through addMessage callback) while adding it to
    ///          the pending requests
    void fetchRequest();

    /// \brief   Process pending requests
    /// \details A pending request is a request that was successfully transfered to the EtherCAT
    /// bus, waiting for its completion.
