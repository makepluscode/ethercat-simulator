...
class Frame {
  public:
    ...
    void addDatagram(uint8_t index, enum Command command, uint32_t address, void const* data,
                     uint16_t data_size);

    /// \brief   Reset the internal datagram pointers to beginning of the frame and be ready to
    /// read/write a new frame. \details This context is used to iterate through the datagrams in
    /// the frame.
    void resetContext();
    ...
    ///          It shall be cleared when done writing and when done reading, to increment the len
    ///          properly when reusing the frame to add new datagrams and not expect inconsistent
    ///          frame size.
    void clear();

    /// \brief finalize the frame to sent it on the network - handle padding and multiple datagram
    /// flag \return the number of bytes to write on the network
    int32_t finalize();

    /// \brief helper to get access on next datagram
    /// \return a tuple with a pointer on the datagram header, a pointer on the datagram data, the
    /// working counter
    ...
    /// \return a tuple with a pointer on the datagram header, a pointer on the datagram data, a
    /// pointer on the working counter.
    ...
    /// \return true if the frame is full (no more datagram can be added: either there is not
    /// sufficient space or max datagram was reached)
    bool isFull() const;

    /// \return true if datagram can be extracted, false otherwise
    bool isDatagramAvailable() const {
        return is_datagram_available_;
    }
    void setIsDatagramAvailable() {
  ...
  private:
    ...
    uint8_t* first_datagram_; // First datagram of the frame - immutable (but non const for move()
                              // semantic)
