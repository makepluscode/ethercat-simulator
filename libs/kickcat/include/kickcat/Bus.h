...
class Bus
{
  public:
    ...
    void waitForState(
        State request, nanoseconds timeout, std::function<void()> background_task = []() {});
    ...
    // It enable users to do one or multiple operations in a row, process something, and process all
    // awaiting frames.
    ...
    void sendMailboxesReadChecks(
        std::function<void(DatagramState const&)> const&
            error); // Fetch in  mailboxes states (full/empty) of compatible slaves
    void sendMailboxesWriteChecks(
        std::function<void(DatagramState const&)> const&
            error); // Fetch out mailboxes states (full/empty) of compatible slaves
    ...
    void readSDO(Slave& slave, uint16_t index, uint8_t subindex, Access CA, void* data,
                 uint32_t* data_size, nanoseconds timeout = 1s);
    void writeSDO(Slave& slave, uint16_t index, uint8_t subindex, bool CA, void* data,
                  uint32_t data_size, nanoseconds timeout = 1s);

    /// \brief  Add a gateway message to the bus
    /// \param  raw_message         A raw EtherCAT mailbox message
    /// \param  raw_message_size    Size of the mailbox message (shall be less or equal of the
    /// actual storage size) \param  gateway_index       Index to link the request to the client in
  ...
  protected: // for unit testing
    ...
    template <typename T> std::tuple<DatagramHeader const*, T const*, uint16_t> nextDatagram();
    ...
    void fetchEeprom();
    bool areEepromReady();
    void readEeprom(uint16_t address, std::vector<Slave*> const& slaves,
                    std::function<void(Slave&, uint32_t word)> apply);
