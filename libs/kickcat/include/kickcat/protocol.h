...
using EthernetFrame =
    std::array<uint8_t, ETH_MAX_SIZE>; // Definition of an Ethernet frame (maximal size)
...
constexpr uint16_t FMMU =
    0x600; // each FFMU entry is described in 16 bytes (6x0 to 6xF), up to 16 FMMU

constexpr uint16_t SYNC_MANAGER =
    0x800; // each SyncManager is described in 8 bytes, up to 8 Sync Manager
...
{
    uint16_t opcode : 7, incomplete : 1, reserved : 8;
    uint16_t index;
} __attribute__((__packed__));
...
// MAC addresses are not used by EtherCAT but set them helps the debug easier when following a
// network trace.
