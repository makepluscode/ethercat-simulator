...
#define THROW_ERROR_CODE(msg, code)                                                                \
    (throw kickcat::ErrorCode{LOCATION(": " msg), static_cast<int32_t>(code)})
#define THROW_ERROR_DATAGRAM(msg, state) (throw kickcat::ErrorDatagram{LOCATION(": " msg), state})
#define THROW_SYSTEM_ERROR_CODE(msg, code)                                                         \
    (throw std::system_error(code, std::generic_category(), LOCATION(": " msg)))
