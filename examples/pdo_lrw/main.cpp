...
int main()
{
    ...
    // LWR: 논리 메모리에 쓰기
    link->addDatagram(::kickcat::Command::LWR, /*logical*/0x20, wbuf, static_cast<uint16_t>(sizeof(wbuf)), process_ok, on_error);
    link->processDatagrams();

    // LRD: 논리 메모리에서 읽기
    uint8_t rbuf[8] = {0};
    link->addDatagram(::kickcat::Command::LRD, /*logical*/0x20, nullptr, static_cast<uint16_t>(sizeof(rbuf)),
