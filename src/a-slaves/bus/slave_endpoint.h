...
class SlaveEndpoint {
public:
    explicit SlaveEndpoint(std::string endpoint)
        : endpoint_(std::move(endpoint)) {}

    void setSlaveCount(std::size_t n) { slave_count_ = n; }
...
public:
    void setConnectionCallback(std::function<void(bool)> cb) { on_connection_ = std::move(cb); }
};
