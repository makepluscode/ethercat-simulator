...
namespace ethercat_sim::slaves {

class El1258Slave : public ethercat_sim::simulation::VirtualSlave {
public:
    explicit El1258Slave(uint16_t station_addr,
...
protected:
    ...
    {
        switch (index) {
            case 0x1000: value = 0x00000000; return true; // device type (placeholder)
            case 0x1008: value = 0; return true;         // name (expedited 0)
            case 0x1009: value = 0; return true;         // HW version
            case 0x100A: value = 0; return true;         // SW version
            case 0x6002: // aggregate inputs
                value = currentAggregate_(); return true;
            case 0x6000: // per-channel
                if (subindex >= 1 && subindex <= 8) {
                    value = (currentAggregate_() >> (subindex - 1)) & 0x1u; return true;
                }
                break;
            case 0x8000: // invert mask
                value = invert_mask_; return true;
            case 0x8001: // debounce ms
                value = debounce_ms_; return true;
            default: break;
        }
        return false;
    }
    ...
    bool onSdoDownload(uint16_t index, uint8_t subindex, uint32_t value, uint8_t nbytes) noexcept override
    {
        ...
        switch (index) {
            case 0x8000: // invert mask
                invert_mask_ = (value & 0xFFu); return true;
            case 0x8001: // debounce
                debounce_ms_ = value; return true;
            case 0x1A00: // TxPDO mapping (Sub0=count, others=mappings)
                if (subindex == 0) { map_count_ = static_cast<uint8_t>(value & 0xFF); return true; }
                if (subindex >= 1 && subindex <= 8) { mappings_[subindex - 1] = value; return true; }
                break;
            case 0x1C13: // assign (Sub0=count, Sub1=0x1A00)
                if (subindex == 0) { assign_count_ = static_cast<uint8_t>(value & 0xFF); return true; }
                if (subindex == 1) {
...
private:
    ...
    {
        uint32_t raw = 0;
        for (int i = 0; i < 8; ++i) raw |= (channels_[i] ? (1u << i) : 0u);
